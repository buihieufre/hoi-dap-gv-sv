...
import { setIO } from "@/shared/socket-io";
      ...
      async (
        ...
        try {
          ...
          if (question.authorId) {
            io.to(`user:${question.authorId}`).emit("answer:new", optimisticAnswer);
          }
          cb?.({ ok: true });
              ...
              Array.from(recipients).map(async (recipientId) => {
                try {
                  ...
                  try {
                    ...
                    console.error(`[Socket] ✗ FCM token lookup failed:`, fcmErr);
                  }
                } catch (err: any) {
                  console.error(
      ...
      async (
        ...
        try {
          ...
          if (answer.question.approvalStatus !== "APPROVED") {
            cb?.({
              ok: false,
              error: "Không thể chỉnh sửa câu trả lời cho câu hỏi chưa được duyệt",
            });
            return;
          }
          ...
          io.to(`question:${questionId}`).emit("answer:updated", optimisticPayload);
          cb?.({ ok: true });
          ...
          try {
            ...
            io.to(`question:${questionId}`).emit("answer:updated", persistedPayload);
          } catch (dbError: any) {
            console.error("[Socket] ✗ Error persisting answer update:", dbError);
